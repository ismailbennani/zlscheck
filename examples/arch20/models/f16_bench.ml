(* The Zélus compiler, version 2.0
  (jeudi 4 juin 2020, 17:57:12 (UTC+0200)) *)
open Ztypes
type state__1753 =
F16_bench_Done_80
| F16_bench_Pull_79 | F16_bench_Roll_78 | F16_bench_Start_77 
open MyOp
open Discrete_obs_fad
open F16_types
type ('a) _f16_phi =
  { mutable i_386 : 'a }

let f16_phi  = 
  let Node { alloc = i_386_alloc; step = i_386_step ; reset = i_386_reset } = 
  Discrete_obs_fad.always_timed 15.  in
  let f16_phi_alloc _ =
    ();{ i_386 = i_386_alloc () (* discrete *)  } in
  let f16_phi_reset self  =
    (i_386_reset self.i_386 :unit) in 
  let f16_phi_step self ((tstep_204:MyOp.t) , (alt_203:MyOp.t)) =
    (i_386_step self.i_386
       (tstep_204 , (Discrete_obs_fad.q_gt (alt_203 , (MyOp.make 0.)))):
    MyOp.t) in
  Node { alloc = f16_phi_alloc; reset = f16_phi_reset ; step = f16_phi_step }
type ('b1 ,
      'a1 ,
      'z ,
      'y ,
      'x ,
      'w ,
      'v ,
      'u ,
      't ,
      's ,
      'r ,
      'q ,
      'p ,
      'o ,
      'n , 'm , 'l , 'k , 'j , 'i , 'h , 'g , 'f , 'e , 'd , 'c , 'b , 'a) _f16 =
  { mutable i_388 : 'b1 ;
    mutable i_387 : 'a1 ;
    mutable i_381 : 'z ;
    mutable m_379 : 'y ;
    mutable m_377 : 'x ;
    mutable m_375 : 'w ;
    mutable m_373 : 'v ;
    mutable m_371 : 'u ;
    mutable m_369 : 't ;
    mutable m_367 : 's ;
    mutable m_365 : 'r ;
    mutable m_363 : 'q ;
    mutable m_361 : 'p ;
    mutable m_359 : 'o ;
    mutable m_357 : 'n ;
    mutable m_355 : 'm ;
    mutable m_353 : 'l ;
    mutable m_351 : 'k ;
    mutable m_349 : 'j ;
    mutable m_347 : 'i ;
    mutable m_345 : 'h ;
    mutable r_342 : 'g ;
    mutable s_341 : 'f ;
    mutable state_263 : 'e ;
    mutable wings_are_level_384 : 'd ;
    mutable rate_is_small_383 : 'c ;
    mutable radsFromWingsLevel_382 : 'b ; mutable radsFromNoseLevel_385 : 'a }

let f16 (tstep_206:float) = 
  let Node { alloc = i_388_alloc; step = i_388_step ; reset = i_388_reset } = f16_phi 
   in 
  let Node { alloc = i_387_alloc; step = i_387_step ; reset = i_387_reset } = Subf16_model.derivatives 
   in
  let f16_alloc _ =
    ();
    { i_381 = (false:bool) ;
      m_379 = (create ():MyOp.t) ;
      m_377 = (create ():MyOp.t) ;
      m_375 = (create ():MyOp.t) ;
      m_373 = (create ():MyOp.t) ;
      m_371 = (create ():MyOp.t) ;
      m_369 = (create ():MyOp.t) ;
      m_367 = (create ():MyOp.t) ;
      m_365 = (create ():MyOp.t) ;
      m_363 = (create ():MyOp.t) ;
      m_361 = (create ():MyOp.t) ;
      m_359 = (create ():MyOp.t) ;
      m_357 = (create ():MyOp.t) ;
      m_355 = (create ():MyOp.t) ;
      m_353 = (create ():MyOp.t) ;
      m_351 = (create ():MyOp.t) ;
      m_349 = (create ():MyOp.t) ;
      m_347 = (create ():MyOp.t) ;
      m_345 = (42.:float) ;
      r_342 = (false:bool) ;
      s_341 = (F16_bench_Done_80:state__1753) ;
      state_263 = (GcasAutopilot.Done:GcasAutopilot.state) ;
      wings_are_level_384 = (false:bool) ;
      rate_is_small_383 = (false:bool) ;
      radsFromWingsLevel_382 = (create ():MyOp.t) ;
      radsFromNoseLevel_385 = (create ():MyOp.t);
      i_388 = i_388_alloc () (* discrete *)  ;
      i_387 = i_387_alloc () (* discrete *)  } in
  let f16_reset self  =
    ((self.i_381 <- true ;
      i_388_reset self.i_388  ;
      self.r_342 <- false ;
      self.s_341 <- F16_bench_Start_77 ; i_387_reset self.i_387 ):unit) in 
  let f16_step self (inp_205:(MyOp.t)Array.t) =
    ((let (phi_209:MyOp.t) = Array.get inp_205  0 in
      let (theta_212:MyOp.t) = Array.get inp_205  1 in
      let (psi_210:MyOp.t) = Array.get inp_205  2 in
      let (initial_cond_220:F16_types.solver_state) =
          { F16_types.f16 =
            { F16_types.vt = (MyOp.make 540.) ;
              F16_types.alpha = (MyOp.scale Constants.dtor  2.1215) ;
              F16_types.beta = (MyOp.make 0.) ;
              F16_types.phi = phi_209 ;
              F16_types.theta = theta_212 ;
              F16_types.psi = psi_210 ;
              F16_types.p = (MyOp.make 0.) ;
              F16_types.q = (MyOp.make 0.) ;
              F16_types.r = (MyOp.make 0.) ;
              F16_types.x9 = (MyOp.make 0.) ;
              F16_types.x10 = (MyOp.make 0.) ;
              F16_types.alt = (MyOp.make 4040.) ;
              F16_types.power = (MyOp.make 9.) } ;
            F16_types.nz = (MyOp.make 0.) ;
            F16_types.ps = (MyOp.make 0.) ; F16_types.ny_r = (MyOp.make 0.) } in
      let (x0_339:MyOp.t) = initial_cond_220.F16_types.ny_r in
      (if self.i_381 then self.m_379 <- x0_339) ;
      (let (x0_335:MyOp.t) = initial_cond_220.F16_types.ps in
       (if self.i_381 then self.m_377 <- x0_335) ;
       (let (x0_331:MyOp.t) = initial_cond_220.F16_types.nz in
        (if self.i_381 then self.m_375 <- x0_331) ;
        (let (x0_327:MyOp.t) = initial_cond_220.F16_types.f16.F16_types.power in
         (if self.i_381 then self.m_373 <- x0_327) ;
         (let (x0_323:MyOp.t) = initial_cond_220.F16_types.f16.F16_types.alt in
          (if self.i_381 then self.m_371 <- x0_323) ;
          (let (x0_319:MyOp.t) = initial_cond_220.F16_types.f16.F16_types.x10 in
           (if self.i_381 then self.m_369 <- x0_319) ;
           (let (x0_315:MyOp.t) = initial_cond_220.F16_types.f16.F16_types.x9 in
            (if self.i_381 then self.m_367 <- x0_315) ;
            (let (x0_311:MyOp.t) = initial_cond_220.F16_types.f16.F16_types.r in
             (if self.i_381 then self.m_365 <- x0_311) ;
             (let (x0_307:MyOp.t) =
                  initial_cond_220.F16_types.f16.F16_types.q in
              (if self.i_381 then self.m_363 <- x0_307) ;
              (let (x0_303:MyOp.t) =
                   initial_cond_220.F16_types.f16.F16_types.p in
               (if self.i_381 then self.m_361 <- x0_303) ;
               (let (x0_299:MyOp.t) =
                    initial_cond_220.F16_types.f16.F16_types.psi in
                (if self.i_381 then self.m_359 <- x0_299) ;
                (let (x0_295:MyOp.t) =
                     initial_cond_220.F16_types.f16.F16_types.theta in
                 (if self.i_381 then self.m_357 <- x0_295) ;
                 (let (x0_291:MyOp.t) =
                      initial_cond_220.F16_types.f16.F16_types.phi in
                  (if self.i_381 then self.m_355 <- x0_291) ;
                  (let (x0_287:MyOp.t) =
                       initial_cond_220.F16_types.f16.F16_types.beta in
                   (if self.i_381 then self.m_353 <- x0_287) ;
                   (let (x0_283:MyOp.t) =
                        initial_cond_220.F16_types.f16.F16_types.alpha in
                    (if self.i_381 then self.m_351 <- x0_283) ;
                    (let (x0_279:MyOp.t) =
                         initial_cond_220.F16_types.f16.F16_types.vt in
                     (if self.i_381 then self.m_349 <- x0_279) ;
                     (let (x0_261:MyOp.t) = MyOp.make 0. in
                      (if self.i_381 then self.m_347 <- x0_261) ;
                      (if self.i_381 then self.m_345 <- tstep_206) ;
                      self.i_381 <- false ;
                      (let (x_380:MyOp.t) = self.m_379 in
                       let (x_378:MyOp.t) = self.m_377 in
                       let (x_376:MyOp.t) = self.m_375 in
                       let (x_374:MyOp.t) = self.m_373 in
                       let (x_372:MyOp.t) = self.m_371 in
                       let (x_370:MyOp.t) = self.m_369 in
                       let (x_368:MyOp.t) = self.m_367 in
                       let (x_366:MyOp.t) = self.m_365 in
                       let (x_364:MyOp.t) = self.m_363 in
                       let (x_362:MyOp.t) = self.m_361 in
                       let (x_360:MyOp.t) = self.m_359 in
                       let (x_358:MyOp.t) = self.m_357 in
                       let (x_356:MyOp.t) = self.m_355 in
                       let (x_354:MyOp.t) = self.m_353 in
                       let (x_352:MyOp.t) = self.m_351 in
                       let (x_350:MyOp.t) = self.m_349 in
                       let (f16_state_231:F16_types.f16_state) =
                           { F16_types.vt = x_350 ;
                             F16_types.alpha = x_352 ;
                             F16_types.beta = x_354 ;
                             F16_types.phi = x_356 ;
                             F16_types.theta = x_358 ;
                             F16_types.psi = x_360 ;
                             F16_types.p = x_362 ;
                             F16_types.q = x_364 ;
                             F16_types.r = x_366 ;
                             F16_types.x9 = x_368 ;
                             F16_types.x10 = x_370 ;
                             F16_types.alt = x_372 ; F16_types.power = x_374 } in
                       let (x_245:F16_types.solver_state) =
                           { F16_types.f16 = f16_state_231 ;
                             F16_types.nz = x_376 ;
                             F16_types.ps = x_378 ; F16_types.ny_r = x_380 } in
                       let (tstep'_213:MyOp.t) = MyOp.make tstep_206 in
                       let (rob_211:MyOp.t) =
                           i_388_step self.i_388
                             (tstep'_213 , x_245.F16_types.f16.F16_types.alt) in
                       let (outp_208:(MyOp.t)Array.t) =
                           Array.of_list ((::) (x_245.F16_types.f16.F16_types.alt
                                                ,
                                                ((::) (x_245.F16_types.f16.F16_types.alpha
                                                       ,
                                                       ((::) (x_245.F16_types.f16.F16_types.beta
                                                              ,
                                                              ((::) (
                                                                    x_245.F16_types.f16.F16_types.vt
                                                                    ,
                                                                    (
                                                                    (::) 
                                                                    (
                                                                    x_245.F16_types.f16.F16_types.phi
                                                                    ,
                                                                    (
                                                                    (::) 
                                                                    (
                                                                    x_245.F16_types.f16.F16_types.theta
                                                                    ,
                                                                    (
                                                                    (::) 
                                                                    (
                                                                    x_245.F16_types.f16.F16_types.psi
                                                                    ,
                                                                    (
                                                                    (::) 
                                                                    (
                                                                    x_245.F16_types.f16.F16_types.p
                                                                    ,
                                                                    (
                                                                    (::) 
                                                                    (
                                                                    x_245.F16_types.f16.F16_types.q
                                                                    ,
                                                                    (
                                                                    (::) 
                                                                    (
                                                                    x_245.F16_types.f16.F16_types.r
                                                                    , 
                                                                    ([]))))))))))))))))))))) in
                       let (alpha_253:MyOp.t) =
                           x_245.F16_types.f16.F16_types.alpha in
                       let (phi_255:MyOp.t) =
                           x_245.F16_types.f16.F16_types.phi in
                       let (theta_256:MyOp.t) =
                           x_245.F16_types.f16.F16_types.theta in
                       let (p_254:MyOp.t) = x_245.F16_types.f16.F16_types.p in
                       let (x_348:MyOp.t) = self.m_347 in
                       (begin match self.s_341 with
                              | F16_bench_Start_77 ->
                                  (if self.r_342 then ()) ;
                                  self.state_263 <- GcasAutopilot.Start ;
                                  (begin match (((>) x_348 
                                                     GcasAutopilot.man_start)
                                                ,
                                                ((&&) ((&&) ((>) x_348 
                                                                 GcasAutopilot.man_start)
                                                            
                                                            ((<) (MyOp.abs 
                                                                    p_254) 
                                                                 GcasAutopilot.eps_p))
                                                      
                                                      ((<) (MyOp.abs 
                                                              (MyOp.(-) 
                                                                 phi_255 
                                                                 (MyOp.( * ) 
                                                                    Constants.pi
                                                                    
                                                                    (
                                                                    F16_utils.round 
                                                                    (
                                                                    MyOp.(/) 
                                                                    phi_255 
                                                                    Constants.pi)))))
                                                           
                                                           GcasAutopilot.eps_phi))
                                                ,
                                                ((&&) ((>) x_348 
                                                           GcasAutopilot.man_start)
                                                      
                                                      ((>) (MyOp.(-) 
                                                              (MyOp.(-) 
                                                                 theta_256 
                                                                 alpha_253) 
                                                              (MyOp.( * ) 
                                                                 GcasAutopilot.twopi
                                                                 
                                                                 (F16_utils.round 
                                                                    (
                                                                    MyOp.(/) 
                                                                    (
                                                                    MyOp.(-) 
                                                                    theta_256
                                                                    
                                                                    alpha_253)
                                                                    
                                                                    GcasAutopilot.twopi))))
                                                           
                                                           GcasAutopilot.path_goal))) with
                                         | (_ , _ , true) ->
                                             self.r_342 <- true ;
                                             self.s_341 <- F16_bench_Done_80
                                         | (_ , true , _) ->
                                             self.r_342 <- true ;
                                             self.s_341 <- F16_bench_Pull_79
                                         | (true , _ , _) ->
                                             self.r_342 <- true ;
                                             self.s_341 <- F16_bench_Roll_78
                                         | _ -> self.r_342 <- false  end)
                              | F16_bench_Roll_78 ->
                                  (if self.r_342 then ()) ;
                                  self.state_263 <- GcasAutopilot.Roll ;
                                  self.radsFromWingsLevel_382 <- F16_utils.round 
                                                                   (MyOp.(/) 
                                                                    phi_255 
                                                                    Constants.pi)
                                  ;
                                  self.rate_is_small_383 <- (<) (MyOp.abs 
                                                                   p_254) 
                                                                GcasAutopilot.eps_p
                                  ;
                                  self.wings_are_level_384 <- (<) (MyOp.abs 
                                                                    (
                                                                    MyOp.(-) 
                                                                    phi_255 
                                                                    (
                                                                    MyOp.( * ) 
                                                                    Constants.pi
                                                                    
                                                                    self.radsFromWingsLevel_382)))
                                                                  
                                                                  GcasAutopilot.eps_phi
                                  ;
                                  (begin match (&&) self.wings_are_level_384 
                                                    self.rate_is_small_383 with
                                         | true ->
                                             self.r_342 <- true ;
                                             self.s_341 <- F16_bench_Pull_79
                                         | _ -> self.r_342 <- false  end)
                              | F16_bench_Pull_79 ->
                                  (if self.r_342 then ()) ;
                                  self.state_263 <- GcasAutopilot.Pull ;
                                  self.radsFromNoseLevel_385 <- F16_utils.round 
                                                                  (MyOp.(/) 
                                                                    (
                                                                    MyOp.(-) 
                                                                    theta_256
                                                                    
                                                                    alpha_253)
                                                                    
                                                                    GcasAutopilot.twopi)
                                  ;
                                  (begin match (>) (MyOp.(-) (MyOp.(-) 
                                                                theta_256 
                                                                alpha_253) 
                                                             (MyOp.( * ) 
                                                                GcasAutopilot.twopi
                                                                
                                                                self.radsFromNoseLevel_385))
                                                    GcasAutopilot.path_goal with
                                         | true ->
                                             self.r_342 <- true ;
                                             self.s_341 <- F16_bench_Done_80
                                         | _ -> self.r_342 <- false  end)
                              | F16_bench_Done_80 ->
                                  (if self.r_342 then ()) ;
                                  self.state_263 <- GcasAutopilot.Done ;
                                  self.r_342 <- false
                               end) ;
                       (let (u_ref_265:F16_types.ref_ctrl) =
                            GcasAutopilot.get_u_ref (self.state_263 ,
                                                     F16_types.defaultCtrlLimits
                                                     ,
                                                     LowLevelController.x_equil
                                                     , x_245.F16_types.f16) in
                        let ((x_ctrl_267:F16_types.solver_state) ,
                             (u_deg_266:F16_types.input)) =
                            LowLevelController.get_u_deg (F16_types.defaultCtrlLimits
                                                          ,
                                                          u_ref_265.F16_types.thtlc_ref
                                                          , x_245) in
                        let ((xd_model_270:F16_types.f16_state) ,
                             (ny_268:MyOp.t) , (nz_269:MyOp.t)) =
                            i_387_step self.i_387
                              (x_245.F16_types.f16 , u_deg_266) in
                        let (ps_271:MyOp.t) =
                            MyOp.(+) (MyOp.( * ) x_ctrl_267.F16_types.f16.F16_types.p
                                                 
                                                 (MyOp.cos x_ctrl_267.F16_types.f16.F16_types.alpha))
                                     
                                     (MyOp.( * ) x_ctrl_267.F16_types.f16.F16_types.r
                                                 
                                                 (MyOp.sin x_ctrl_267.F16_types.f16.F16_types.alpha)) in
                        let (ny_r_272:MyOp.t) =
                            MyOp.(+) ny_268 
                                     x_ctrl_267.F16_types.f16.F16_types.r in
                        let ((nzd_274:MyOp.t) ,
                             (psd_275:MyOp.t) , (ny_rd_273:MyOp.t)) =
                            LowLevelController.get_integrator_derivatives 
                              (u_ref_265 , nz_269 , ps_271 , ny_r_272) in
                        let (xd_248:F16_types.solver_state) =
                            { F16_types.f16 = xd_model_270 ;
                              F16_types.nz = nzd_274 ;
                              F16_types.ps = psd_275 ;
                              F16_types.ny_r = ny_rd_273 } in
                        let (dx_337:MyOp.t) = xd_248.F16_types.ny_r in
                        self.m_379 <- MyOp.(+) x_380 
                                               (MyOp.( * ) tstep'_213  dx_337)
                        ;
                        (let (dx_333:MyOp.t) = xd_248.F16_types.ps in
                         self.m_377 <- MyOp.(+) x_378 
                                                (MyOp.( * ) tstep'_213 
                                                            dx_333) ;
                         (let (dx_329:MyOp.t) = xd_248.F16_types.nz in
                          self.m_375 <- MyOp.(+) x_376 
                                                 (MyOp.( * ) tstep'_213 
                                                             dx_329) ;
                          (let (dx_325:MyOp.t) =
                               xd_248.F16_types.f16.F16_types.power in
                           self.m_373 <- MyOp.(+) x_374 
                                                  (MyOp.( * ) tstep'_213 
                                                              dx_325) ;
                           (let (dx_321:MyOp.t) =
                                xd_248.F16_types.f16.F16_types.alt in
                            self.m_371 <- MyOp.(+) x_372 
                                                   (MyOp.( * ) tstep'_213 
                                                               dx_321) ;
                            (let (dx_317:MyOp.t) =
                                 xd_248.F16_types.f16.F16_types.x10 in
                             self.m_369 <- MyOp.(+) x_370 
                                                    (MyOp.( * ) tstep'_213 
                                                                dx_317) ;
                             (let (dx_313:MyOp.t) =
                                  xd_248.F16_types.f16.F16_types.x9 in
                              self.m_367 <- MyOp.(+) x_368 
                                                     (MyOp.( * ) tstep'_213 
                                                                 dx_313) ;
                              (let (dx_309:MyOp.t) =
                                   xd_248.F16_types.f16.F16_types.r in
                               self.m_365 <- MyOp.(+) x_366 
                                                      (MyOp.( * ) tstep'_213 
                                                                  dx_309) ;
                               (let (dx_305:MyOp.t) =
                                    xd_248.F16_types.f16.F16_types.q in
                                self.m_363 <- MyOp.(+) x_364 
                                                       (MyOp.( * ) tstep'_213
                                                                    dx_305) ;
                                (let (dx_301:MyOp.t) =
                                     xd_248.F16_types.f16.F16_types.p in
                                 self.m_361 <- MyOp.(+) x_362 
                                                        (MyOp.( * ) tstep'_213
                                                                     
                                                                    dx_301) ;
                                 (let (dx_297:MyOp.t) =
                                      xd_248.F16_types.f16.F16_types.psi in
                                  self.m_359 <- MyOp.(+) x_360 
                                                         (MyOp.( * ) 
                                                            tstep'_213 
                                                            dx_297) ;
                                  (let (dx_293:MyOp.t) =
                                       xd_248.F16_types.f16.F16_types.theta in
                                   self.m_357 <- MyOp.(+) x_358 
                                                          (MyOp.( * ) 
                                                             tstep'_213 
                                                             dx_293) ;
                                   (let (dx_289:MyOp.t) =
                                        xd_248.F16_types.f16.F16_types.phi in
                                    self.m_355 <- MyOp.(+) x_356 
                                                           (MyOp.( * ) 
                                                              tstep'_213 
                                                              dx_289) ;
                                    (let (dx_285:MyOp.t) =
                                         xd_248.F16_types.f16.F16_types.beta in
                                     self.m_353 <- MyOp.(+) x_354 
                                                            (MyOp.( * ) 
                                                               tstep'_213 
                                                               dx_285) ;
                                     (let (dx_281:MyOp.t) =
                                          xd_248.F16_types.f16.F16_types.alpha in
                                      self.m_351 <- MyOp.(+) x_352 
                                                             (MyOp.( * ) 
                                                                tstep'_213 
                                                                dx_281) ;
                                      (let (dx_277:MyOp.t) =
                                           xd_248.F16_types.f16.F16_types.vt in
                                       self.m_349 <- MyOp.(+) x_350 
                                                              (MyOp.( * ) 
                                                                 tstep'_213 
                                                                 dx_277) ;
                                       (let (dx_259:MyOp.t) = MyOp.make 1. in
                                        self.m_347 <- MyOp.(+) x_348 
                                                               (MyOp.( * ) 
                                                                  tstep'_213 
                                                                  dx_259) ;
                                        (let (x_346:float) = self.m_345 in
                                         self.m_345 <- (+.) x_346  tstep_206
                                         ;
                                         (x_346 ,
                                          outp_208 ,
                                          (MyOp.translate rob_211  10.)))))))))))))))))))))))))))))))))))))):
    float * MyOp.t Array.t * MyOp.t) in
  Node { alloc = f16_alloc; reset = f16_reset ; step = f16_step }
