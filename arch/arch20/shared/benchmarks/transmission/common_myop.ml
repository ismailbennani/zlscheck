...
let lookup_engine ((l_73:MyOp.t) , (c_72:MyOp.t)) =
  At_utils.lookup2D (Consts.line_engine ,
                     Consts.col_engine , Consts.vals_engine)  (l_73 , c_72)

let lookup_interpup ((l_75:MyOp.t) , (c_74:MyOp.t)) =
  At_utils.lookup2D (Consts.line_interpup ,
                     Consts.col_interpup , Consts.vals_interpup) 
                    (l_75 , c_74)

let lookup_interpdn ((l_77:MyOp.t) , (c_76:MyOp.t)) =
  At_utils.lookup2D (Consts.line_interpdn ,
                     Consts.col_interpdn , Consts.vals_interpdn) 
                    (l_77 , c_76)

let lookup_factork (l_78:MyOp.t) =
  At_utils.lookup1D (Consts.line_factork , Consts.vals_factork)  l_78

let lookup_torkratio (l_79:MyOp.t) =
  At_utils.lookup1D (Consts.line_torkratio , Consts.vals_torkratio)  l_79

let lookup_gear (l_80:MyOp.t) =
  At_utils.lookup1D (Consts.line_gear , Consts.vals_gear)  l_80
